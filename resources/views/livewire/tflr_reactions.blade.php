<div class="reactions-component" x-data="{ showPicker: false, showList: false }">
    <div class="flex flex-row-reverse items-center justify-between gap-4">
        ...
        <div class="relative">
            <!-- Primary Button -->
            <button x-ref="likeBtn" @keydown.escape="showPicker = false" @mouseenter="showPicker = true"
                @mouseleave="showPicker = false" @click="showPicker = !showPicker"
                @keydown.enter.space.prevent="showPicker = !showPicker" type="button"
                :aria-expanded="showPicker.toString()" aria-haspopup="menu"
            ...
            <template x-teleport="body">
                <div x-show="showPicker" x-cloak role="menu" @keydown.right="$focus.wrap().next()"
                    @keydown.left="$focus.wrap().previous()" @keydown.escape="showPicker = false; $refs.likeBtn.focus()"
                    x-trap.noscroll="showPicker" x-anchor.top-end.offset.8="$refs.likeBtn" x-transition
                    @mouseenter="showPicker = true" @mouseleave="showPicker = false" x-init="() => { if (showPicker) $nextTick(() => $el.querySelector('button').focus()) }"
                    class="z-50">
                    <div
                        ...
                        <div class="flex items-center gap-2">
                            @foreach ($reactionTypes as $type => $config)
                                @if (method_exists($this->getModel(), 'canReact') && !$this->getModel()->canReact($type))
                                    @continue
                                @endif
                                <button @keydown.enter.space.prevent="react('{{ $type }}')"
                                    @keydown.escape="showPicker = false; $refs.likeBtn.focus()"
                                    wire:click="react('{{ $type }}')" type="button" role="menuitemradio"
                                    :aria-checked="'{{ $type }}'
                                    === '{{ $userReaction }}'"
            ...
            <template x-if="$userReaction">
                <div role="status" aria-live="polite" class="sr-only" x-text="`You reacted with ${$userReaction}`">
                </div>
            </template>
        </div>
        ...
        @if ($this->totalReactions > 0)
            <div class="relative">
                <button x-ref="countBtn" @keydown.escape="showList = false; $wire.call('closeReactionsList')"
                    ...
                    type="button" :aria-expanded="showList.toString()"
                    :aria-controls="'reactions-dialog-{{ $this->modelId }}'" aria-haspopup="dialog"
                ...
                <template x-teleport="body">
                    <div x-show="showList" x-trap.noscroll="showList" x-cloak
                        @click.away="showList = false; $wire.call('closeReactionsList')"
                        id="reactions-dialog-{{ $this->modelId }}" role="dialog" tabindex="-1" aria-modal="true"
                        aria-labelledby="reactions-dialog-title-{{ $this->modelId }}"
                        aria-describedby="reactions-dialog-description-{{ $this->modelId }}"
                        x-anchor.bottom-start.offset.8="$refs.countBtn" x-transition
                        class="z-50 min-w-[320px] bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700"
                        x-data="{
                            focusFirstTab() {
                                    $nextTick(() => {
                                        const firstTab = $refs.allTab ?? $el.querySelector('[data-tab]');
                                        firstTab && firstTab.focus();
                                    });
                                },
                                tabs() { return Array.from($el.querySelectorAll('[data-tab]')); },
                                focusNextTab() {
                                    const t = this.tabs();
                                    if (!t.length) return;
                                    const active = document.activeElement;
                                    const idx = Math.max(0, t.indexOf(active));
                                ...
                                focusFirstRow() {
                                    $nextTick(() => {
                                        const firstRow = $refs.usersList?.querySelector('[data-user-row]');
                                        firstRow && firstRow.focus();
                                    });
                                },
                                focusRow(delta) {
                                    const rows = Array.from($refs.usersList?.querySelectorAll('[data-user-row]') || []);
                                    if (!rows.length) return;
                                    const active = document.activeElement;
                                    const idx = Math.max(0, rows.indexOf(active));
                                    const next = rows[(idx + delta + rows.length) % rows.length];
                                    next && next.focus();
                                }
                        }" x-effect="showList && focusFirstTab()">
                        ...
                        @if ($showReactionsList)
                            ...
                            <div role="tablist" aria-label="{{ __('Filter reactions by type') }}"
                                class="flex items-center gap-1 p-2 border-b border-gray-200 dark:border-gray-700">
                                <button x-ref="allTab" data-tab type="button" role="tab"
                                    :aria-selected="@js($selectedReactionFilter === null) ? 'true' : 'false'"
                                    @keydown.right.prevent.stop="focusNextTab()"
                                    @keydown.left.prevent.stop="focusPrevTab()"
                                    @keydown.enter.prevent.stop="($event.target.click(), focusFirstRow())"
                                    @keydown.space.prevent.stop="($event.target.click(), focusFirstRow())"
                                    @keydown.escape.prevent.stop="focusNextTab()"
                                ...
                                @foreach ($reactionTypes as $type => $config)
                                    ...
                                    @endif
                                    @if ($reactions[$type] > 0)
                                        <button data-tab type="button" role="tab"
                                            :aria-selected="@js($selectedReactionFilter === $type) ? 'true' : 'false'"
                                            @keydown.right.prevent.stop="focusNextTab()"
                                            @keydown.left.prevent.stop="focusPrevTab()"
                                            @keydown.enter.prevent.stop="($event.target.click(), focusFirstRow())"
                                            @keydown.space.prevent.stop="($event.target.click(), focusFirstRow())"
                            ...
                            <!-- Users list -->
                            <div x-ref="usersList" class="p-3 max-h-96 overflow-y-scroll no-scrollbar" role="listbox"
                                aria-live="polite" :aria-busy="$wire.isLoadingReactions"
                                aria-label="{{ __('List of users who reacted with :reaction', ['reaction' => $reactionTypes[$selectedReactionFilter]['label'] ?? 'reactions']) }}"
                                @keydown.down.prevent.stop="focusRow(1)" @keydown.up.prevent.stop="focusRow(-1)"
                                @keydown.escape.prevent.stop="focusNextTab()">
                                    ...
                                    <div class="space-y-2">
                                        @foreach ($reactionUsers as $reactionUser)
                                            ...
                                            @if (isset($reactionTypes[$reactionUser['type']]))
                                                <div data-user-row tabindex="0" role="option"
                                        ...
                                        <!-- Load more spinner -->
                                        <div wire:key="loadMore" class="max-w-6xl h-6 mx-auto"
                                            x-intersect.full="$wire.loadMore();">
                                            <div wire:loading wire:target="loadMore" role="status"
                                                aria-live="assertive" class="flex w-full justify-center py-6">
                                                <span class="sr-only">Loading more reactions...</span>
                                                <div class="flex items-center gap-3 px-6 py-3">
                                                    <svg aria-hidden="true" class="animate-spin h-5 w-5 text-blue-500"
                                                    ...
                                                    <span class="text-gray-700 font-medium">Loading more
                                                        reactions...</span>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                @else
                                    <p class="text-sm text-gray-500 dark:text-gray-400 text-center py-4">No reactions
                                    </p>
                                @endif
                            </div>
                        @endif
